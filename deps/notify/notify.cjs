...
AnimationQueue.prototype.animate = function (object) {
	if (object === undefined || object.func === undefined)
		return;
	const self = this;
	object.func.apply(null, object.args)
		.then(() => {
			if (self.queue.length > 0) {
				// Run next animation
				self.animate(self, self.queue.shift());
			} else {
				self.running = false;
			}
		})
...
let config = {
	...
	defaultWindow: {
		...
		webPreferences: {
			preload: path.join(__dirname, 'notifypreload.cjs'),
...
function updateTemplatePath() {
	const templatePath = path.join(__dirname, 'notification.html');
...
function showNotification(notificationObj) {
	return new Promise(resolve => {
		...
		if (activeNotifications.length < config.maxVisibleNotifications) {
			...
			getWindow().then(notificationWindow => {
				...
				const closeFunc = buildCloseNotification(notificationWindow, notificationObj, () => {
					return timeoutId;
				});
				const closeNotificationSafely = buildCloseNotificationSafely(closeFunc);
				...
				// Set contents, ...
				notificationWindow.webContents.send('electron-notify-set-contents', JSON.stringify(notificationObj));
				// Show window
...
ipc.on('electron-notify-close', (event, winId, notificationObj) => {
	const closeFunc = buildCloseNotification(BrowserWindow.fromId(winId), notificationObj);
	buildCloseNotificationSafely(closeFunc)('close');
});

ipc.on('electron-notify-click', (event, winId, notificationObj) => {
...
function moveOneDown(startPos) {
	return new Promise(resolve => {
		...
		asyncFunc(notificationPosArray, moveNotificationAnimation, () => {
			resolve();
		});
	});
}
...
function closeAll() {
	...
	_.forEach(activeNotifications, window => {
		window.close();
	});
	_.forEach(inactiveWindows, window => {
		window.close();
	});
	// Reset certain vars
	nextInsertPos = {};
